...
- `/ble/advertisement` 提供 Pocket 4 和 Pocket 4 Pro 两种 BLE 广播样本；
...
素材目录如果包含 `sdcard/` 和 `internal/` 子目录，会分别映射到两个存储；没有子目录时全部映射到内置存储。启动日志是 JSON 行，包含文件数量和端口，不输出 Wi-Fi 密码。

Luna 开发者模式选择 `Osmo Pocket 4` 或 `Osmo Pocket 4 Pro` 后，使用本地 mock 地址连接。Electron 内置的 mock 启动入口会根据当前 DJI 设备自动启动此服务。
