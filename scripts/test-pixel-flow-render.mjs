...
try {
  ...
  const legacySaturatedInitial = render(compositionWithPixelFlow(composition, { initialSaturation: 100 }), 0)
  ...
  const filterOff = render(compositionWithPixelFlow(composition, { filterStrength: 0 }), 0.42)
  const filterFull = render(compositionWithPixelFlow(composition, { filterStrength: 100 }), 0.42)
  const finalBaseFilterOff = render(compositionWithPixelFlow(composition, { filterStrength: 0, flowStrength: 0, bloomStrength: 0 }), duration)
  const finalBaseFilterFull = render(compositionWithPixelFlow(composition, { filterStrength: 100, flowStrength: 0, bloomStrength: 0 }), duration)
  ...
  assert.deepEqual(legacySaturatedInitial.data, initial.data, 'legacy initial saturation must not change the monochrome plate')
  ...
  const activeFilterDifference = frameDifference(filterOff, filterFull)
  assert.ok(activeFilterDifference > 0.01, `the Hertz color strength changes only the active pixel-flow light (${activeFilterDifference})`)
  assert.deepEqual(finalBaseFilterOff.data, finalBaseFilterFull.data, 'the Hertz color strength must not re-grade the revealed source image')
