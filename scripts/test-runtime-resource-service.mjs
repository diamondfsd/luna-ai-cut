...
try {
  ...
  assert.equal(await getRuntimeResourceCachePath(repairedRoot, repaired.definition), null, '缺少必要文件时缓存必须失效')
  await loadRuntimeResource(repairedRoot, repaired.definition, { fetcher: repairFetcher })
  assert.equal((await stat(path.join(repairedPath, 'repair.json'))).size, Buffer.byteLength('{"ok":true}'))
  await writeFile(path.join(repairedPath, 'repair.cube'), 'changed-size')
  assert.equal(await getRuntimeResourceCachePath(repairedRoot, repaired.definition), null, '文件大小变化时缓存必须失效')
  await loadRuntimeResource(repairedRoot, repaired.definition, { fetcher: repairFetcher })
  assert.equal((await readFile(path.join(repairedPath, 'repair.cube'), 'utf8')), 'original')
  assert.equal(repairFetches, 3, '损坏缓存每次都应重建')
