...
发布说明必须以“上一个正式版本 tag 到当前发布 commit 的实际差异”为准，不以开发分支上的计划、未合入代码或未来版本内容为准。发布前先确认变更范围：

```bash
PREVIOUS_TAG="$(git describe --tags --abbrev=0 "v<版本号>^" 2>/dev/null || true)"
git log --oneline "${PREVIOUS_TAG}..HEAD"
git diff --stat "${PREVIOUS_TAG}..HEAD"
```

- 只有当前版本首次对用户可用的能力，才能放在“新功能”。
- 已在上一个正式版本中可用的能力，其优化放在“改进”；只有确实修复了已发布功能的问题，才能放在“Bug 修复”。
- 不能把尚未进入上一个正式版本产物的功能写入上一个版本的发布说明；发现历史说明超前时，应先更正历史说明，再生成当前版本说明。
- “其他”只保留用户确实需要知道的兼容性、资源或平台限制；版本号、构建命令、内部实现和提交信息不写入面向用户的发布说明。
- 每条说明都必须能回答“用户得到什么变化”，避免只写代码模块、内部流程或实现名词。

发布说明完成后，逐条对照 `PREVIOUS_TAG..HEAD` 检查，删除不属于本次版本或无法从用户角度解释的条目。安装包平台、签名、公证和哈希结果记录在发布执行记录中，不混入用户发布说明。
...
> 客户端启动后每天最多自动查询一次热更新。自动查询不会下载或安装；用户仍需在顶部问号窗口中确认并应用更新，安装包则需要手动检查后下载。
