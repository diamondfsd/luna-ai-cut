...
pub fn run(model_path: &str, rgb: &[u8], mask: &[u8]) -> Result<InpaintResult, String> {
    ...
    let outputs = session.run(ort::inputs!["image" => image, "mask" => mask])
    ...
    if shape.len() != 4 || shape[0] != 1 || shape[1] != 3 || shape[2] != INPUT_SIZE as i64 || shape[3] != INPUT_SIZE as i64 {
    ...
    for pixel in 0..PIXELS {
        for channel in 0..3 {
            output[pixel * 3 + channel] = values[channel * PIXELS + pixel].round().clamp(0.0, 255.0) as u8;
        }
    }
    Ok(InpaintResult { rgb: output, model_load_ms, inference_ms })
}
