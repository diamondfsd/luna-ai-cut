...
pub fn segment(
    ...
    let result = catch_unwind(AssertUnwindSafe(|| crate::sam_core::segment(
        vision_encoder_path,
        prompt_decoder_path,
        rgb.as_ref(),
        source_width,
        ...
        let message = panic.downcast_ref::<&str>().map(|value| (*value).to_string())
    ...
    Ok(SamSegmentationResult { width: result.width, height: result.height, bytes: result.bytes.into() })
}
