...
当前已接入：工作台照片导出支持 PNG/JPEG/WebP，图片格式和分辨率配置会贯通到 WebGPU 静态合成与文件扩展名；真实工作台 E2E 已验证 PNG、4K（3840x2160）和 WebGPU 导出路径。Live 图封面仍固定使用 JPEG。

仍待验收：透明背景、水印与高分辨率复杂图层、含实际 EXIF 的原图元数据保留，以及大文件取消和磁盘空间错误。
...
当前限制：不支持的音频编码仍需要后续统一重编码为 AAC；长视频和 4K 压力验证仍待在真实设备上采集内存、吞吐和取消耗时数据。
