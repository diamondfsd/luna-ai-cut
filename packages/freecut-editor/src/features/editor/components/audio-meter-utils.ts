import type { AudioItem, CompositionItem, TimelineItem, TimelineTrack } from '@freecut/types/timeline'
import type { Transition } from '@freecut/types/transition'
import { getAudioClipFadeMultiplier, getAudioFadeMultiplier } from '@freecut/shared/utils/audio-fade-curve'
import {
  getManagedLinkedAudioTransitions,
  hasLinkedAudioCompanion,
  isCompositionAudioItem,
} from '@freecut/shared/utils/linked-media'
import { timelineToSourceFrames } from '@freecut/features/editor/deps/timeline-utils'
import { getTrackKind, resolveEffectiveTrackStates } from '@freecut/features/editor/deps/timeline-utils'
import {
...
} from '@freecut/features/editor/deps/composition-runtime'
...
export type AudioMeterCompositionLookup = Record<
  string,
  {
    id: string
...
export function subscribeLiveOverrideVersion(callback: () => void): () => void {
  ...
  return () => {
    liveOverrideListeners.delete(callback)
  }
}
  ...
  const clipFadeMultiplier = segment.clipFadeSpans
    ...
    : getAudioFadeMultiplier({
        ...
        contentStartOffsetFrames:
          'contentStartOffsetFrames' in segment ? segment.contentStartOffsetFrames : 0,
        contentEndOffsetFrames:
          'contentEndOffsetFrames' in segment ? segment.contentEndOffsetFrames : 0,
...
function appendCompositionWrapperSources(params: {
  ...
  const nestedFrame =
    sourceStart + timelineToSourceFrames(localFrame, wrapper.speed ?? 1, fps, sourceFps)
...
function appendCompoundSegmentSources(params: {
  ...
  const nestedFrame =
    segment.trimBefore + timelineToSourceFrames(localFrame, segment.playbackRate, fps, sourceFps)
  ...
  appendAudioMeterSources({
    ...
    fps: composition.fps,
    gainMultiplier:
      gainMultiplier * Math.max(0, toLinearGain(segment.volumeDb) * crossfadeMultiplier),
    depth: depth + 1,
    compositionsById,
    ownerTrackId: ownerTrackId ?? wrapper.trackId,
    sources,
...
function appendAudioMeterSources(params: {
  ...
  const managedLinkedAudioItems = audioItems.filter(
    (item) => !isCompositionAudioItem(item) && managedLinkedAudioIds.has(item.id),
  )
  const managedLinkedAudioItemsById = new Map(
    managedLinkedAudioItems.map((item) => [item.id, item]),
  )
  const managedLinkedAudioTransitionDefs = managedLinkedAudioTransitions.flatMap(
  ...
  const standaloneAudioItems = audioItems.filter(
    (item) => !isCompositionAudioItem(item) && !managedLinkedAudioIds.has(item.id),
  )
  const videoAudioItems = renderPlan.videoItems.filter(
  ...
  const committedTrackVolumeByTrackId = new Map(
    effectiveTracks.map((track) => [track.id, toLinearGain(track.volume ?? 0)]),
  )
  ...
  for (const segment of directSegments) {
    ...
    appendDirectSegmentSources({
      ...
      gainMultiplier,
      trackVolumeGain:
        trackVolumeByTrackId.get(sourceTrackId) !== undefined
          ? (trackVolumeByTrackId.get(sourceTrackId) ?? 1) /
            Math.max(committedTrackVolumeByTrackId.get(sourceTrackId) ?? 1, 0.000001)
          : 1,
      segments: [segment],
      sources,
      trackId,
  ...
  const compoundAudioItems = audioItems.filter((item): item is EnrichedCompositionAudioItem =>
  ...
  const managedCompoundAudioItemsById = new Map(
    managedCompoundAudioItems.map((item) => [item.id, item]),
  )
  const managedCompoundAudioTransitionDefs = managedLinkedAudioTransitions.flatMap(
    ({ transition, leftAudio, rightAudio }) => {
  ...
  for (const wrapper of standaloneCompoundAudioItems) {
    appendCompositionWrapperSources({
      ...
      wrapper,
      wrapperGain:
        toLinearGain((wrapper.volume ?? 0) + (wrapper.trackVolumeDb ?? 0)) *
  ...
  for (const wrapper of standaloneCompositionVisualItems) {
    appendCompositionWrapperSources({
      ...
      sources,
      wrapper,
      wrapperGain: getTrackGainCorrection(
        wrapper.trackId,
        committedTrackVolumeByTrackId.get(wrapper.trackId) ?? 1,
      ),
    })
  }
}
...
function buildAudioMeterGraphNode(params: {
  ...
  const directSegments = [
    ...
    committedTrackVolumeGain:
      trackVolumeByTrackId.get(directTrackIdByItemId.get(segment.itemId) ?? '') ?? 1,
  }))
...
function appendAudioMeterSourcesFromGraph(params: {
  ...
  for (const directEntry of graphNode.directSegments) {
    appendDirectSegmentSources({
      ...
      fps: graphNode.fps,
      gainMultiplier,
      trackVolumeGain: getTrackGainCorrection(
        directEntry.trackId,
        directEntry.committedTrackVolumeGain,
      ),
  ...
  for (const compoundEntry of graphNode.compoundSegments) {
    ...
    const nestedFrame =
      compoundEntry.segment.trimBefore +
      timelineToSourceFrames(
        localFrame,
  ...
  for (const compositionEntry of graphNode.compositionWrappers) {
    if (
      depth > AUDIO_METER_MAX_DEPTH ||
      compositionEntry.wrapper.muted ||
      !compositionEntry.wrapper.trackVisible
    ) {
      continue
    }
    ...
    const sourceStart =
      compositionEntry.wrapper.sourceStart ?? compositionEntry.wrapper.trimStart ?? 0
    const nestedFrame =
      sourceStart +
      timelineToSourceFrames(
        localFrame,
  ...
  for (const track of params.tracks) {
    ...
    result.set(
      ...
      estimateAudioMeterLevel({
        ...
        waveformsByMediaId: params.waveformsByMediaId,
      }),
    )
  }
