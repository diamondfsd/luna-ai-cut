...
import { FileAccessError } from '@freecut/features/preview/deps/media-library'
import type { TimelineTrack, VideoItem } from '@freecut/types/timeline'
...
vi.mock('@freecut/features/media-library/services/media-library-service', async () => {
  const { FileAccessError } = await import('@freecut/features/preview/deps/media-library')
  return {
    mediaLibraryService,
    FileAccessError,
  }
...
vi.mock('@freecut/features/media-library/services/proxy-service', () => ({
  proxyService: {
...
vi.mock('@freecut/features/media-library/stores/media-library-store', () => ({
...
describe('resolveMediaUrls', () => {
  ...
  it('keeps video audio on the original source when proxy playback is enabled', async () => {
    const { proxyService } = await import('@freecut/features/preview/deps/media-library-contract')
    ;(mediaLibraryService.getMedia as Mock).mockResolvedValue({
      id: 'media-1',
      fileName: 'video.mp4',
    })
