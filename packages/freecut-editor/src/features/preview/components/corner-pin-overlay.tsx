...
import { useTimelineStore } from '@freecut/features/preview/deps/timeline-store'
...
export const CornerPinOverlay = memo(function CornerPinOverlay({
  ...
  const cornerToCanvas = useCallback(
    (corner: CornerPinHandle): [number, number] => {
      ...
      const itemLeft =
        projectSize.width / 2 + itemTransform.x - itemTransform.width / 2 + targetRect.x
      const itemTop =
        projectSize.height / 2 + itemTransform.y - itemTransform.height / 2 + targetRect.y
