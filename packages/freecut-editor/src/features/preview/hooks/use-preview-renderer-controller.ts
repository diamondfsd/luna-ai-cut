...
import { usePreviewBridgeStore, type PostEditWarmRequest } from '@freecut/shared/state/preview-bridge'
...
import {
  ...
  type FrameRange,
} from '@freecut/shared/utils/frame-invalidation'
...
export function usePreviewRendererController({
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const { EffectsPipeline } = await import('@freecut/infrastructure/gpu-effects')
        ...
        if (device) {
          ...
          if (warmPipeline) {
            ...
            try {
              const { TransitionPipeline } = await import('@freecut/infrastructure/gpu-transitions')
