...
import { resetPlaybackPreviewState } from '@freecut/shared/state/playback-preview-test-helpers'
...
vi.mock('@freecut/shared/logging/perf-marks', () => ({
  perfMarkRender: perfMarkMocks.mark,
  withPerfMeasure: (_name: string, callback: () => unknown) => callback(),
}))

vi.mock('@freecut/shared/marquee/use-marquee-selection', () => {
...
describe('TimelineContent playback selection behavior', () => {
  ...
  it('keeps a horizontal wheel gesture horizontal over a track section', () => {
    const frameCallbacks: FrameRequestCallback[] = []
    const animationFrameSpy = vi
      .spyOn(window, 'requestAnimationFrame')
      .mockImplementation((callback) => {
