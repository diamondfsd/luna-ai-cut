...
import { withPerfMeasure, perfMarkRender } from '@freecut/shared/logging/perf-marks'
...
import type { TimelineTrack as TimelineTrackType } from '@freecut/types/timeline'
...
type TrackScrollbarSection = 'single'
...
function TrackSectionScrollbarOverlay({
  ...
  return (
    <div
      className={fill ? 'relative min-h-0 flex-1' : 'relative shrink-0'}
...
const TimelineTrackSectionsSurface = memo(function TimelineTrackSectionsSurface({
  ...
  const renderTrackSection = (
    ...
    <div
      data-track-section={options.section}
      className="relative"
    >
...
export const TimelineContent = memo(function TimelineContent({
  ...
  useLayoutEffect(() => {
    ...
    revealTrackInScrollContainer(allTracksScrollRef?.current ?? null, activeTrackId)
  }, [
    activeTrackId,
    videoTracks.length,
    audioTracks.length,
  ...
  const allTracksHasOverflow = useTrackSectionHasOverflow(
    allTracksScrollRef,
    tracks.length,
    videoTracks.length,
    audioTracks.length,
