...
function hasActiveEditPreview(): boolean {
  ...
  if (
    rolling.trimmedItemId !== null ||
    rolling.neighborItemId !== null ||
    rolling.handle !== null
  ) {
    return true
  }
...
function isPreviewAudioStartupHoldActive(hold: PreviewAudioStartupHold, nowMs: number): boolean {
...
export function isPreviewWorkDeferred(options: { ignoreAudioStartupHold?: boolean } = {}): boolean {
  return (
...
export function registerPreviewAudioStartupHold(
  ...
  const maxDurationMs =
    requestedMaxDurationMs == null ? null : Math.max(minDurationMs, requestedMaxDurationMs)
