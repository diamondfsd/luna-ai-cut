...
function findAdjacentNeighbors(item: TimelineItem, items: TimelineItem[]): NeighborPair {
 ...
 */
export function findNearestNeighbors(item: TimelineItem, items: TimelineItem[]): NeighborPair {
  const itemEnd = item.from + item.durationInFrames
  let leftNeighbor: TimelineItem | null = null
  let rightNeighbor: TimelineItem | null = null
