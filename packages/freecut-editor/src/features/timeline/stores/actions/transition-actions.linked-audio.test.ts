...
import type { AudioItem, CompositionItem, VideoItem } from '@freecut/types/timeline'
import {
...
import { getManagedLinkedAudioTransitions } from '@freecut/shared/utils/linked-media'
...
describe('transition actions with linked audio companions', () => {
  ...
  it('creates an overlap and moves linked companions for untrimmed clips', () => {
    useItemsStore.getState().setItems([
      makeVideoItem({
        id: 'video-1',
        from: 0,
    ...
    expect(addTransition('video-1', 'video-2', 'crossfade', 30)).toBe(true)
    expect(useItemsStore.getState().itemById['video-2']).toMatchObject({ from: 30 })
    expect(useItemsStore.getState().itemById['audio-2']).toMatchObject({ from: 30 })
    expect(useItemsStore.getState().itemById['video-3']).toMatchObject({ from: 90 })
    expect(useItemsStore.getState().itemById['audio-3']).toMatchObject({ from: 90 })
    expect(useTransitionsStore.getState().transitions).toEqual([
