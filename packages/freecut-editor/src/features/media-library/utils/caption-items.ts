...
import type { MediaTranscriptSegment } from '@freecut/types/storage'
import type { MediaCaption } from '@freecut/infrastructure/analysis/media-tagger'
import type { SubtitleCue, SubtitleFormat } from '@freecut/shared/utils/subtitles'
...
interface BuildSubtitleTextItemsForClipOptions {
  ...
  styleTemplate?: CaptionTextItemTemplate
  sourceType?: GeneratedCaptionSource['type']
  sourceMetadata?: Omit<GeneratedCaptionSource, 'type' | 'clipId' | 'mediaId'>
}
...
export function findGeneratedCaptionItemsForClip(
  ...
  return items.filter((item): item is TextItem & { captionSource: GeneratedCaptionSource } => {
    if (!isGeneratedCaptionTextItem(item)) return false
    return (
...
export function buildSubtitleTextItems({
  ...
  return cues.flatMap((cue) => {
    ...
    const defaultCaptionItem: TextItem = {
      ...
      durationInFrames,
      mediaId,
      captionSource: buildCaptionSource(mediaId, clipId, sourceType, {
        fileName,
        format,
        importedAt: Date.now(),
      }),
      label: parsed.plainText.slice(0, 48),
      ...
      color: '#ffffff',
      backgroundColor: 'rgba(0, 0, 0, 0.55)',
      textAlign: parsed.alignment?.textAlign ?? 'center',
      verticalAlign: parsed.alignment?.verticalAlign ?? 'middle',
      lineHeight: 1.15,
      letterSpacing: 0,
      textShadow: {
        offsetX: 0,
...
export function appendVirtualTranscriptCaptionTrack(
  ...
  for (const track of sourceTracks) {
    for (const item of track.items ?? []) {
      ...
      const captionItems = buildSubtitleTextItemsForClip({
      ...
      virtualItems.push(
        ...captionItems.map((captionItem, index) => ({
          ...captionItem,
          id: `virtual-transcript-${item.id}-${index}`,
          trackId: VIRTUAL_TRANSCRIPT_CAPTION_TRACK_ID,
          linkedGroupId: undefined,
          captionSource: captionItem.captionSource
            ? {
                ...captionItem.captionSource,
