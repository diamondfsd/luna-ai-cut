...
type ClipBoundSubtitleSegmentSource = Extract<
  import('@freecut/types/timeline').SubtitleSegmentSource,
  { clipId: string }
>
...
interface BuildSubtitleSegmentForClipOptions {
  ...
  canvasWidth: number
  canvasHeight: number
  source: import('@freecut/types/timeline').SubtitleSegmentSource
  styleTemplate?: CaptionTextItemTemplate
...
export function buildSubtitleSegmentForClip(
...
): import('@freecut/types/timeline').SubtitleSegmentItem | null {
  const {
    clip,
    cues,
    timelineFps,
  ...
  const overlappingCues: import('@freecut/types/timeline').SubtitleSegmentCue[] = []
...
export function consolidateCaptionTextItemsToSegments(
  ...
  segments: import('@freecut/types/timeline').SubtitleSegmentItem[]
  ...
  const segments: import('@freecut/types/timeline').SubtitleSegmentItem[] = []
  ...
  for (const [clipId, group] of byClip) {
    ...
    const segment: import('@freecut/types/timeline').SubtitleSegmentItem = {
      id: crypto.randomUUID(),
      type: 'subtitle',
