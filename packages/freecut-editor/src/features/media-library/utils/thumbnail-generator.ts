...
import { ensureProResDecoderRegistered } from '@freecut/infrastructure/browser/register-prores-decoder'
 ...
 */
async function generateAudioThumbnail(file: File, options: ThumbnailOptions = {}): Promise<Blob> {
  const opts = { ...DEFAULT_THUMBNAIL_OPTIONS, ...options }
 ...
 */
async function generateImageThumbnail(file: File, options: ThumbnailOptions = {}): Promise<Blob> {
  const opts = { ...DEFAULT_THUMBNAIL_OPTIONS, ...options }
 ...
 */
export async function generateThumbnail(file: File, options: ThumbnailOptions = {}): Promise<Blob> {
