import { useMediaLibraryStore } from '@freecut/features/media-library/stores/media-library-store'
import { proxyService } from '@freecut/features/media-library/services/proxy-service'
import { getSharedProxyKey } from '@freecut/features/media-library/utils/proxy-key'
...
export async function resolveMediaUrl(mediaId: string): Promise<string> {
  ...
  const requestPromise = (async () => {
    const { mediaLibraryService, FileAccessError } =
      await import('@freecut/features/media-library/services/media-library-service')
