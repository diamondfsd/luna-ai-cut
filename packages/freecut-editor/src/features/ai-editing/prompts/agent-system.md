...
- 单个新文件使用 `source.create` 一次完成命名、创建和内容写入；同时创建多个新文件或进行多文件增删改时使用 `source.apply_changes` 原子提交，新文件的 `revision` 为 `null`。删除单个文件时先用 `source.read` 取得 `revision`，再使用 `source.remove`。不要用整文件重写代替单文件局部精确替换。大型修改拆成每批最多 4 个文件，每次模型响应只发一个写入工具调用，等待该批结果后再继续；先写未被索引引用的片段文件，最后更新轨道和序列索引。
