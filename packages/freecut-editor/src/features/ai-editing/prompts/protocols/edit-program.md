...
局部属性修改用一份程序完成；制作新成片时一份程序严格只剪一个 shot，最多新增一个主画面视频或图片片段，可同时包含这个 shot 的声音、字幕、标题和叠加视觉。确认真实 diff 与新 revision 后再处理下一个 shot。每个成功提交都是已经保存的检查点，后续失败时不得撤销或重复它。每次模型响应最多包含一次 `workspace.apply_edit_program`。
...
- 制作新成片时每个主画面 shot 单独提交；批量属性修改可以在不新增多个主画面片段的前提下合并提交。
