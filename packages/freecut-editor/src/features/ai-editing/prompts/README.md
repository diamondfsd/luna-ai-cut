...
- `skills/foundations/`: general editing judgment exposed through the skill catalog.
- `skills/`: optional domain knowledge listed by name and description in the initial context, then read by exact name when the current task needs it. Skills can be enabled, disabled, or extended by users.
