...
describe('AI editing system prompt', () => {
  it('includes directly usable common editing formats and reserves docs for extensions', async () => {
    ...
    expect(prompt).toContain('有旁白或清晰口播的成片默认同步制作字幕')
    expect(prompt).toContain('同一成片的重点词强调色保持固定')
    expect(prompt).toContain('caption.style.backgroundColor')
    expect(prompt).toContain('caption.spans')
