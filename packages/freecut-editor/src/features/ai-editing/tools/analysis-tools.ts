...
const searchTranscript = defineAiEditingTool({
  ...
  inputSchema: objectSchema({
    ...
    mediaIds: { type: 'array', items: { type: 'string' }, description: '可选的素材范围，使用 workspace.media[].ref。' },
  }, ['query']),
...
const requestAnalysis = defineAiEditingTool({
  ...
  risk: 'analysis',
  inputSchema: objectSchema({
    mediaIds: { type: 'array', items: { type: 'string' }, description: '要分析的素材，使用 workspace.media[].ref。' },
    ...
    if (args.kind === 'transcript') {
      ...
      for (const [index, item] of eligibleMedia.entries()) {
        ...
        await runMediaTranscriptionJob(item.id, {
          onProgress: (progress) => reportItemProgress(
            index,
            eligibleMedia.length,
            item.fileName,
            '正在识别口播',
