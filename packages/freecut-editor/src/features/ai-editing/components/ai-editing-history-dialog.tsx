...
import { Check, Copy, History, Loader2, MessageSquareText } from 'lucide-react'
...
export function AiEditingHistoryDialog({
  ...
  useEffect(() => {
    ...
    setLoading(true)
    setError(null)
    void listAiEditingConversationHistory(projectId).then((next) => {
      if (!active) return
      setSessions(next)
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="freecut-app dark flex h-[80vh] w-[80vw] max-w-none flex-col gap-0 overflow-hidden p-0">
        <DialogHeader className="shrink-0 border-b border-border px-6 py-5 pr-14">
          ...
          <DialogDescription>查看本项目已归档的剪辑助手对话，不会影响当前会话。</DialogDescription>
        </DialogHeader>
                  ...
                  {sessions.map((session) => (
                    ...
                    >
                      <span className="min-w-0 space-y-1">
                        <span className="block truncate text-xs font-medium">{getSessionTitle(session)}</span>
              ...
              <div className="flex h-full min-h-0 flex-col">
                <div className="flex shrink-0 items-start justify-between gap-4 border-b border-border px-6 py-4">
                  <div className="min-w-0 space-y-1">
                    <p className="truncate text-sm font-medium text-foreground">{getSessionTitle(selectedSession)}</p>
                    <p className="text-xs text-muted-foreground">
                      开始于 {formatTimestamp(selectedSession.createdAt)} · 归档于 {formatTimestamp(selectedSession.archivedAt)}
                    </p>
                  </div>
                  <Button
                    type="button"
                    ...
                    aria-label={copiedSessionId === selectedSession.id ? '已复制会话' : '复制整段会话'}
                    data-tooltip={copiedSessionId === selectedSession.id ? '已复制会话' : '复制整段会话'}
                  >
