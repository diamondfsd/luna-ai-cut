...
- 需要新配音时调用 `audio.generate_speech`，需要背景音乐时调用 `audio.generate_music`。
- 生成工具会保存新的音频素材并返回 `mediaId`。先读取返回的结构化结果，再由模型决定是否调用 `timeline.add_media`、使用哪条轨道、从什么时间开始以及如何设置音量和淡入淡出。
