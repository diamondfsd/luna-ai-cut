...
import { requireWorkspaceRoot } from '@freecut/infrastructure/storage/workspace-fs/root'
import { readJson, writeJsonAtomic } from '@freecut/infrastructure/storage/workspace-fs/fs-primitives'
import { MARKER_FILENAME } from '@freecut/infrastructure/storage/workspace-fs/paths'
import type { WorkspaceMarker } from '@freecut/infrastructure/storage/workspace-fs/bootstrap'
...
import { createProject } from '@freecut/infrastructure/storage/workspace-fs/projects'
import { createMedia } from '@freecut/infrastructure/storage/workspace-fs/media'
import { saveThumbnail } from '@freecut/infrastructure/storage/workspace-fs/thumbnails'
import { associateMediaWithProject } from '@freecut/infrastructure/storage/workspace-fs/project-media'
import { saveGifFrames } from '@freecut/infrastructure/storage/workspace-fs/gif-frames'
import { saveWaveformRecord } from '@freecut/infrastructure/storage/workspace-fs/waveforms'
import { saveDecodedPreviewAudio } from '@freecut/infrastructure/storage/workspace-fs/decoded-preview-audio'
import { saveTranscript } from '@freecut/infrastructure/storage/workspace-fs/transcripts'
...
async function writeMigrationErrors(errors: MigrationReport['errors']): Promise<void> {
  ...
  if (errors.length === 0) {
    ...
    try {
      const { removeEntry } = await import('@freecut/infrastructure/storage/workspace-fs/fs-primitives')
