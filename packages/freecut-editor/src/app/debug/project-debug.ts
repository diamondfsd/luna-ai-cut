...
} from '@freecut/features/project-bundle/types/snapshot'
import type {
  FixtureType,
  FixtureOptions,
  FixtureResult,
} from '@freecut/features/project-bundle/services/test-fixtures'
import type { GpuEffectInstance } from '@freecut/infrastructure/gpu-effects'
import {
  exportProjectJson,
...
} from '@freecut/features/project-bundle/services/json-export-service'
import {
  importProjectFromJsonString,
  importProjectFromClipboard,
  validateSnapshotData,
  showImportFilePicker,
} from '@freecut/features/project-bundle/services/json-import-service'
import {
  validateProject,
  formatValidationErrors,
} from '@freecut/features/project-bundle/schemas/project-schema'
import {
  getAllProjects,
  getProject,
  getAllMedia,
...
function createDebugAPI(): ProjectDebugAPI {
  return {
    ...
    generateFixture: async (type, options) => {
      const { generateFixture } = await import('@freecut/features/project-bundle/services/test-fixtures')
    ...
    listFixtures: async () => {
      const { getAvailableFixtures } =
        await import('@freecut/features/project-bundle/services/test-fixtures')
      return getAvailableFixtures()
    },

    // Playback control (for debugging)
    seekTo: async (frame) => {
      const { usePlaybackStore } = await import('@freecut/shared/state/playback')
    ...
    getItemsAtFrame: async (frame: number) => {
      const { useItemsStore } = await import('@freecut/features/timeline/stores/items-store')
    ...
    stores: async () => {
      const [
        ...
        import('@freecut/shared/state/playback'),
        import('@freecut/features/timeline/stores/items-store'),
        import('@freecut/features/timeline/stores/transitions-store'),
        import('@freecut/features/timeline/stores/timeline-store'),
        import('@freecut/features/media-library/stores/media-library-store'),
      ])
      return {
    ...
    getTransitions: async () => {
      const { useTransitionsStore } = await import('@freecut/features/timeline/stores/transitions-store')
    ...
    getTransitionWindows: async () => {
      ...
      const { resolveTransitionWindows } =
        await import('@freecut/shared/timeline/transitions/transition-planner')
    ...
    getPlaybackState: async () => {
      const [{ usePlaybackStore }, { usePreviewBridgeStore }] = await Promise.all([
        import('@freecut/shared/state/playback'),
        import('@freecut/shared/state/preview-bridge'),
      ])
    ...
    getMediaLibrary: async () => {
      const { useMediaLibraryStore } =
        await import('@freecut/features/media-library/stores/media-library-store')
    ...
    overlaps: async () => {
      const [{ useItemsStore }, { useTransitionsStore }, { detectOverlappingItems }] =
        await Promise.all([
          import('@freecut/features/timeline/stores/items-store'),
          import('@freecut/features/timeline/stores/transitions-store'),
          import('@freecut/features/timeline/utils/collision-utils'),
        ])
    ...
    benchmarkEffect: async (effectType, paramOverrides, opts) => {
      const { EffectsPipeline, getGpuEffect, getGpuEffectDefaultParams, GPU_EFFECT_REGISTRY } =
        await import('@freecut/infrastructure/gpu-effects')
    ...
    previewTrace: {
      start: () => {
        void import('@freecut/shared/logging/preview-trace').then((m) => m.startPreviewTrace())
      },
      stop: () => {
        void import('@freecut/shared/logging/preview-trace').then((m) => m.stopPreviewTrace())
      },
      clear: () => {
        void import('@freecut/shared/logging/preview-trace').then((m) => m.clearPreviewTrace())
      },
      events: () => {
    ...
    captureTransition: async (targetFrame?: number) => {
      const [
        ...
        import('@freecut/features/timeline/stores/items-store'),
        import('@freecut/features/timeline/stores/timeline-store'),
        import('@freecut/shared/timeline/transitions/transition-planner'),
        import('@freecut/shared/logging/preview-trace'),
      ])
