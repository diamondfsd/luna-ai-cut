...
桌面端通用约束：当前计算机环境受操作系统和无线网卡厂商安全策略影响，不能稳定读取当前已连接 SSID。当前 SSID 为空不能判断网络是否连接，也不能作为连接成功、失败、重试或恢复旧网络的标准。蓝牙的作用是读取相机热点自己的 SSID 和密码，平台网络接口使用这组目标凭据完成接入，最终状态由相机网段地址和厂商协议握手确认。

桌面端需要：

1. 通过 BLE 获取相机热点的 SSID/密码；
2. 用平台原生接口连接这组目标凭据，不依赖读取当前 SSID；
3. 等待网络获得本地地址，并探测 `192.168.2.1`；
4. 建立 UDP/TCP/HTTP 会话；
5. 断开相机时只关闭应用持有的会话，不执行旧 Wi-Fi 恢复。
...
- 连接失败、用户取消、AP 未出现、切网后协议不可达时给出可行动的错误；
- 记录连接阶段、网卡、BSSID、IP、端口和错误码；不以当前 SSID 判断状态，也不在断开时恢复旧 Wi-Fi。

验收：不手工查看或输入密码即可加入相机 AP，能持续访问相机地址；断开后应用不再管理 Wi-Fi，若系统仍停留在相机热点，由用户自行切换网络。
...
7. 相机 AP 在浏览或下载中掉线；
8. 下载文件校验和断点恢复；
9. 退出应用时 BLE、UDP 和 Wi-Fi 会话清理，不执行旧 Wi-Fi 恢复；
...
| Wi-Fi 切网影响用户网络 | 用户失去互联网或应用访问错误网卡 | 显示阶段、记录网卡状态、探测相机地址、失败时提示用户 |
...
3. 应用连接 DJI 时可以临时切换系统 Wi-Fi，但断开后不恢复原网络；这是桌面端区别于 Android `bindProcessToNetwork` 的产品约束。
...
3. 完成 macOS/Windows Electron Web Bluetooth 长连接实机验收，先实现通过蓝牙取目标 SSID/密码，不读取当前 SSID 作为连接标准；
...
当前仍明确保留的硬件工作：真实 macOS/Windows Electron Web Bluetooth 长连接、使用蓝牙凭据完成系统 Wi-Fi 切换和协议确认，以及 Pocket 4P 真实掉线后的断点恢复。当前非 loopback 地址会给出明确错误，不会伪装成已支持的真实连接。
