...
The Luna integration downloads fixed model files from ModelScope into the host-selected
cache root. See scripts/weights.py for the manifest and verification rules.
"""
...
DIT_REL = {d: dit_rel(d) for d in ("sm-music", "sm-sfx")}   # fp32 (picker)
DEC_REL = {"same-s": dec_rel("same-s")}
ENC_REL = {"same-s": enc_rel("same-s")}
T5_REL = "models/tflite/t5gemma/encoder_fp16.tflite"
DEFAULT_DECODER = {"sm-music": "same-s", "sm-sfx": "same-s"}
...
def valid_T_lat(seconds):
    ...
    TFLite pick the SAME length -> no length-driven divergence. The DiT handles odd L
    natively; SAME-S (even T_aud=L*16 % 32) pads odd->even at encode/decode and trims."""
...
def main():
    ...
    # Models
    ap.add_argument("--dit", choices=list(DIT_REL.keys()), default=None,
                    help="DiT model (sm-music or sm-sfx). If omitted, prompts "
                         "interactively with an arrow-key picker.")
    ap.add_argument("--decoder", choices=list(DEC_REL.keys()), default=None,
                    help="Audio decoder. The small models use SAME-S. "
    ...
    ap.add_argument("--encoder-precision", choices=list(PRECISIONS), default=None,
                    help="Override --precision for the SAME encoder (audio-to-audio / "
                         "inpainting only). The Luna runtime uses the SAME-S encoder.")
    ...
    ap.add_argument("--lora", action="append", nargs="+", default=None,
                    ...
                    help="A LoRA adapter to merge into the DiT — repeat the flag to stack "
                         ...
                         "written into a cached copy of the DiT under the configured cache root and "
    ...
    tok = P.Tokenizer(ensure_local("models/tokenizer.model"))
