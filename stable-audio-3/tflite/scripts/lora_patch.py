...
mechanism (validated on the Stable Audio DiT graphs):
...
def get_patched_dit(base_path, specs, *, family: str, precision: str,
    ...
    # Prefer the host-provided cache root so patched multi-GB models follow the
    # same user-selected storage boundary as the base weights.
    cache_dir = Path(cache_dir or os.environ.get("SA3_LORA_CACHE_ROOT", "")
                     or (Path(base_path).parent.parent / "lora-cache"))
