...
def print_example_commands(header: str | None = None) -> None:
    ...
    have = {name: bundle_status(name) == (5, 5) for name in ("sm-music", "sm-sfx")}
    ...
    one_dit = "sm-music" if have["sm-music"] else "sm-sfx"
    one_dec = "same-s"
    ...
    if missing:
        ...
        print(f"        Re-run {BOLD}./install.sh{RESET} to pick them up, or just use them in"
              f" {BOLD}./sa3{RESET} —")
        print(f"        weights auto-download from ModelScope on first use.")
