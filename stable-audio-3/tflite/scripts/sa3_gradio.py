...
is wired, plus a precision picker the MLX/TRT UIs don't need:
  - Model picker: sm-music / sm-sfx (hot-swap; the DiT interpreter
  ...
  - Precision picker: the Luna release uses fp32 TFLite graphs for CPU inference.
...
Launch:
    ...
    ./sa3-gradio --dit sm-sfx --precision w8a32
...
from sa3_tflite import (  # noqa: E402
    ...
    COND_TOKENS, COND_DIM, SAMPLE_RATE, SAMPLES_PER_LATENT, MIN_SIGMA,
)
...
DEFAULT_CACHE_ROOT = Path(os.environ.get("SA3_CACHE_ROOT", str(REPO))).resolve()
OUTPUT_DIR = Path(os.environ.get("SA3_OUTPUT_ROOT", str(DEFAULT_CACHE_ROOT / "output" / "gradio")))
...
LORAS_DIR = Path(os.environ.get("SA3_LORA_ROOT", str(DEFAULT_CACHE_ROOT / "loras")))
LORA_DIR_NAMES = {"sm-sfx": "sa3-sm-sfx", "sm-music": "sa3-sm-music"}
...
MAX_SECONDS = {"sm-music": 30, "sm-sfx": 30}
...
# large and holds its conditioning as RESIDENT tensors, so
...
_dit_lru: list[str] = []
_DIT_CACHE_MAX = 1          # keep one large DiT resident at a time
...
def get_t5():
    ...
    if _t5 is None:
        ...
        _tok = P.Tokenizer(ensure_local("models/tokenizer.model"))
    return _t5, _tok
...
def get_dit(dit_path, T_lat, t5_hidden, t5_mask, seconds, cfg, apg,
    ...
    while len(_dit_cache) >= _DIT_CACHE_MAX:
        old = _dit_lru.pop(0)
        _log(f"  ← LRU-evicting DiT {Path(old).name}")
...
def run_generation(dit_name: str, decoder_name: str, precision: str, prompt: str,
    ...
    if lora_specs:
        ...
        dit_path = get_patched_dit(dit_path, lora_specs, family=dit_name,
                                   precision=precision, log=lambda m: _log(f"  {m}"))
    ...
    latents = P.sample(
        ...
        None,
        None,
        on_step=lambda completed, total: report("sampling", completed / max(total, 1)),
...
def _lora_specs_from_ui(vals, notes):
    ...
    from lora_core import LoraError, parse_lora_spec
...
def _meta_suffix(entry) -> str:
    ...
    if dit:
        parts.append({"sm-music": "sm-mus", "sm-sfx": "sm-sfx"}.get(dit, dit))
