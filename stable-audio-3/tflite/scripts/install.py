...
2. Ask which DiT bundles to download, then fetch them from ModelScope.
...
def main() -> None:
    ...
    ap.add_argument("--download", default="",
                    ...
                    help="Comma-separated list of bundles to pre-download "
                         "(sm-music, sm-sfx). Without this flag, "
                         "nothing is downloaded — sa3_tflite.py will fetch any "
                         "missing weights from ModelScope on first use.")
    cli = ap.parse_args()
    ...
    if chosen:
        ...
        for name in chosen:
            ...
            for rel, _remote in items:
                if rel in seen:
                    continue
                seen.add(rel)
                ensure_local(rel)
    else:
        step("No --download set — weights will auto-download from ModelScope on first ./sa3 use")
        print(f"  To pre-download instead, pass:  {Path(sys.executable).name} install.py --download sm-music")
        print(f"                              or:  ./install.sh --download sm-music,sm-sfx")
