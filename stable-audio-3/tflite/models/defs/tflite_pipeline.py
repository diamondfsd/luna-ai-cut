...
# This file lives in <project>/models/defs/. The default tokenizer path is kept for
# standalone callers; Luna passes the verified manifest path under its cache root.
...
class Tokenizer:
    """SentencePiece front-end. The caller supplies the verified tokenizer path.
    Encode(prompt)[:256], pad=0 reproduces the SA3 groundtruth input_ids exactly."""
    def __init__(self, model_path=TOKENIZER_MODEL):
        ...
        if not model_path.exists():
            raise FileNotFoundError(
                ...
                f"release under models/tokenizer.model or the configured cache root."
            )
