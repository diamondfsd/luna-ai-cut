// Adapted for Luna's worker protocol from FunASR's MIT-licensed funasr-paraformer.
...
static std::vector<float> run_graph(ggml_context*c,ggml_tensor*out,ggml_tensor*in1,const float*d1,ggml_tensor*in2,const float*d2){
  ...
  ggml_backend_cpu_set_n_threads(be,g_threads);ggml_backend_graph_compute(be,gf);
...
static size_t utf8_length(const std::string &value) {
  size_t length = 0;
...
static bool read_stdin_audio(std::vector<float> &samples) {
#ifdef _WIN32
  if (_setmode(_fileno(stdin), _O_BINARY) == -1) return false;
#endif
  float buffer[16 * 1024];
  samples.clear();
  for (;;) {
    size_t count = fread(buffer, sizeof(float), sizeof(buffer) / sizeof(buffer[0]), stdin);
...
int main(int argc,char**argv){
  ...
  g_threads=std::max(1,std::min(16,atoi(argv[4])));
  const int vad_maxseg=30000;
  ...
  if(vocab.empty()){fprintf(stderr,"字幕识别模型缺少词表\n");return 1;}
  emit_line("{\"version\":1,\"type\":\"ready\",\"modelLoadMs\":"+std::to_string((ggml_time_us()-load_started)/1000)+",\"gpu\":false}");
  ...
  // Full pipeline (fbank -> CMVN -> encoder -> CIF -> decoder) on one VAD region.
  auto run_seg=[&](const std::vector<float>& wav)->std::vector<timed_text>{
    ...
    for(int t=0;t<L;t++){
      ...
      if(fire){ acoustic.insert(acoustic.end(),frame.begin(),frame.end()); fire_frames.push_back(t); integrate-=1.0f;
    ...
    std::vector<int> text_ids, text_frames;
    for(int n=0;n<N;n++){
      if(seg_ids[n]==1||seg_ids[n]==2||seg_ids[n]<0||seg_ids[n]>=(int)vocab.size())continue;
      text_ids.push_back(seg_ids[n]);
      text_frames.push_back(fire_frames[n]);
    }
    std::vector<std::pair<int,int>> ranges;
    for(int begin=0;begin<(int)text_ids.size();){
      int end=begin+1;
      while(end<(int)text_ids.size()){
        std::vector<int> candidate(text_ids.begin()+begin,text_ids.begin()+end+1);
        int duration=(text_frames[end]-text_frames[begin])*LFR_N*10;
        if(utf8_length(detok_pf(candidate,vocab))>16||duration>4000)break;
        end++;
      }
      ranges.push_back({begin,end});
      begin=end;
    }
    std::vector<timed_text> result;
    int duration_ms=(int)((int64_t)wav.size()*1000/FS);
    for(int i=0;i<(int)ranges.size();i++){
      int begin=ranges[i].first,end=ranges[i].second;
      std::vector<int> ids(text_ids.begin()+begin,text_ids.begin()+end);
      std::string text=detok_pf(ids,vocab);if(text.empty())continue;
      int start=i==0?0:(text_frames[begin-1]+text_frames[begin])*LFR_N*5;
      int finish=i+1==(int)ranges.size()?duration_ms:(text_frames[end-1]+text_frames[end])*LFR_N*5;
      start=std::max(0,std::min(std::max(0,duration_ms-10),start));
      finish=std::max(start+10,std::min(duration_ms,finish));
      result.push_back({start,finish,text});
    }
    return result;
  };

  int64_t t0=ggml_time_us();
  std::vector<float>wav;if(!read_stdin_audio(wav)){fprintf(stderr,"无法读取视频语音\n");return 1;}
  std::vector<std::pair<int,int>> segs;
  if(!funasr_vad_segments(vad_path,wav,vad_maxseg,segs,g_threads)){fprintf(stderr,"无法分析语音停顿\n");return 1;}
  int emitted=0;
  for(auto&s:segs){
    int off=(int)((int64_t)s.first*16000/1000), end=(int)((int64_t)s.second*16000/1000);
    if(end>(int)wav.size())end=wav.size(); if(end-off<WINLEN)continue;
    std::vector<float> seg(wav.begin()+off,wav.begin()+end);
    for(const auto& chunk:run_seg(seg)){
      emit_line("{\"version\":1,\"type\":\"segment\",\"startMs\":"+std::to_string(source_start_ms+s.first+chunk.start_ms)+",\"endMs\":"+std::to_string(source_start_ms+s.first+chunk.end_ms)+",\"text\":\""+json_escape(chunk.text)+"\"}");
      emitted++;
    }
    emit_line("{\"version\":1,\"type\":\"progress\",\"processedMs\":"+std::to_string(std::min<uint64_t>(total_ms,s.second))+",\"totalMs\":"+std::to_string(total_ms)+"}");
  }
  uint64_t audio_ms=std::min<uint64_t>(total_ms,(uint64_t)wav.size()*1000/FS);
  emit_line("{\"version\":1,\"type\":\"complete\",\"language\":\"zh\",\"audioMs\":"+std::to_string(audio_ms)+",\"inferenceMs\":"+std::to_string((ggml_time_us()-t0)/1000)+",\"segmentCount\":"+std::to_string(emitted)+"}");
