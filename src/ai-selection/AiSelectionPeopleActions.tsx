...
import { Button, IconButton, LoadingIndicator, Slider, Switch, Tooltip } from '../ui'
...
export function AiSelectionPeopleActions({
  ...
  useEffect(() => { setThresholdValue(faceGroupingThreshold) }, [faceGroupingThreshold])

  function commitThreshold(value: number): void {
    setThresholdValue(value)
    if (value !== faceGroupingThreshold) void onFaceGroupingThresholdChange(value)
  }
  ...
  return <>
    <header className="ai-selection-view-heading">
      ...
      <div className="ai-selection-view-actions ai-selection-people-actions">
        <div className="ai-selection-people-threshold"><span>分组阈值</span><Slider value={thresholdValue} min={0.4} max={0.6} step={0.01} ariaLabel="人物分组阈值" disabled={busy} onValueChange={setThresholdValue} onValueCommit={commitThreshold} /><output>{thresholdValue.toFixed(2)}</output></div>
