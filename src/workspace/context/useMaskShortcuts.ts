...
export function useMaskShortcuts(options: Options): void {
  const {
    ...
    removeActiveComponent, setSemanticPicking, setManualTool, setShowOverlay, setBrushSize,
  } = options
  useEffect(() => {
    if (!editing) return
  ...
  }, [busy, cancelSegmentation, editing, hasActiveComponent, removeActiveComponent, semanticPicking, setBrushSize, setManualTool, setSemanticPicking, setShowOverlay])
}
