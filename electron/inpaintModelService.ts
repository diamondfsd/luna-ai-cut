...
export async function getInpaintModelCacheStatus(): Promise<{ cached: boolean; sizeBytes: number }> {
  ...
  const cached = await Promise.all(candidates.map(async (candidate) => {
    const info = await stat(candidate).catch(() => null)
    return info?.isFile() === true && info.size === INPAINT_MODEL.sizeBytes
  })).then((values) => values.some(Boolean))
  return { cached, sizeBytes: INPAINT_MODEL.sizeBytes }
}
...
export async function loadInpaintModel(
  ...
  if (!pending) {
    pending = (async () => {
      ...
      try {
        return await loadVerifiedModelFile(modelDir, INPAINT_MODEL, { signal, onProgress })
      } catch (error) {
