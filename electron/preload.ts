...
const lunaApi: LunaApi & { exportTask: LunaExportTaskApi } = {
  ...
  workspace: {
    ...
    generateReferenceMatchAiLut: (request: import('../src/shared/types').WorkspaceReferenceMatchAiLutRequest) => ipcRenderer.invoke('workspace:generateReferenceMatchAiLut', request),
    ...
    exportRenderedLivePhoto: (name: string, imagePath: string, videoPath: string, appleLivePhoto: boolean, preserveInputs?: boolean, recordTask?: boolean, coverTimeSeconds?: number, outputDir?: string) => ipcRenderer.invoke('workspace:exportRenderedLivePhoto', name, imagePath, videoPath, appleLivePhoto, preserveInputs, recordTask, coverTimeSeconds, outputDir),
