...
源文件应按职责保持清晰，避免把页面、功能组件、业务模块、服务和样式域无边界地堆在同一个文件中。新增或修改功能时，优先按职责拆分为可复用组件、独立模块、服务或功能样式文件；只有当拆分会制造紧耦合、重复抽象或降低可读性时，才保留在同一文件中。提交前检查相关改动范围的模块边界、依赖方向和复用性，历史规格文档、外部资料和生成文件可按其来源组织。
