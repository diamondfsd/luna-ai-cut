...
- 任务需要加载 `openai-docs` 时，使用本地 `agent-browser` 命令搜索和访问 OpenAI 官方文档，不使用内置浏览器控制链路。
- 该规则仅适用于 OpenAI 文档查询；Electron 应用验证仍遵循下方 Playwright Test 约定，不得使用 `agent-browser` 代替 Electron E2E。

## AI 助手架构约束

**严禁宿主代码根据自然语言文案推断用户意图或替模型决定工作流。** 用户要咨询、设计方案、确认方案、执行编辑、继续处理还是结束任务，必须由大模型结合完整会话自行判断。Harness 只提供能力和执行环境，不能成为基于关键词的第二套意图模型。

- 禁止使用关键词、正则表达式、固定短语、字符串包含关系、前后缀或相似的启发式规则，对用户输入或模型回复进行意图分类。例如不得在代码中识别“可以”“就按这个来”“方案”“脚本”“删除”“开始”等文案并据此切换执行模式。
- 禁止根据自然语言文案创建或推进宿主工作流状态，包括但不限于“等待方案确认”“已确认执行”“直接编辑”“仅文本回答”“延迟回复”和“任务已完成”。不得通过扩充关键词表修补误判。
- 禁止修改、包装或追加用户消息来表达宿主猜测出的意图。发送给模型的用户消息必须保持用户原文；资源引用等结构化上下文应通过明确、稳定的协议提供，不得伪装成用户要求。
- 禁止 Harness 根据回复文案、工具名称或业务关键词替模型决定继续、收尾或完成。模型返回工具调用时执行工具并回传结构化结果；模型按协议返回最终答复时结束。工具成功后不得跳过模型自行宣告完成。
- Harness 只负责协议解析与修复、工具注册和参数校验、工具执行、结果分页、取消、超时、总轮次与单轮调用数量等资源边界，以及未提交修改、工具错误等客观状态记录。
- 可以保留不参与意图推断的确定性文本处理，例如协议字段解析、用户明确调用搜索工具后的字幕关键词匹配，以及路径、扩展名、MIME、ID 和源码格式校验。这些逻辑不得反向影响用户意图、任务类型或完成状态。
- 需要模型遵守的业务行为应放在系统提示词、工具描述或结构化协议中，并由模型判断何时适用；数据安全约束仍必须在类型、Schema、工具和 Store 写入边界实施。
- 修改 AI 对话、编排或 Harness 时，提交前至少进行三类扫描：用户输入文案判断、模型回复文案判断、基于自然语言改变停止或工具策略的逻辑。发现此类代码必须删除，不得保留 fallback 或历史兼容分支。
- 对上述架构边界增加回归测试时，应验证“工具结果返回模型并等待模型下一步”和“用户消息原样传递”等协议行为，不得为具体自然语言短语建立意图分类测试。

## 维护规范

按清晰的功能边界和职责合理拆分模块。相对独立的功能、组件、服务职责或样式域应放在独立文件中，不要把无关逻辑集中堆放在同一个文件里；是否拆分以可读性、内聚性和维护成本为准，不设置固定行数上限。

### 测试约定

- 本项目是 Electron 桌面应用。执行 `pnpm dev` 时必须以 Electron 窗口成功打开为开发入口；Vite 地址仅供 Electron 内部加载，无法作为应用功能入口直接访问，不得向用户确认、报告或要求访问该地址。
...
- 用户没有明确要求时，不得运行 `pnpm run build:app`；需要验证时优先运行与改动直接相关的类型检查、Lint 和自动化测试。
...
- 纯 UI 样式、间距、尺寸、控件位置和静态文案调整，不得因此新增或扩展单元测试、组件测试或 Electron E2E；使用类型检查、变更范围 Lint、构建和代码审查验证。只有调整涉及有分支的交互行为、持久化或已发生的回归风险时才增加行为测试。
...
4. 仅在用户明确要求时运行 `pnpm run build:app`；用户未明确要求时不得因 UI 改动自动执行该命令。
...
- 当前项目仍处于开发阶段。编辑数据结构或轨道规范发生变化时，不为本地测试项目增加历史数据迁移、自动修复或兼容分支。
- 旧测试数据与新规范冲突时，直接清空对应测试项目的时间线数据后按新规范重新生成。
- 数据约束必须在类型、编译或 Store 写入边界实施，不能只依赖 AI 提示词，也不能用加载时迁移掩盖违规写入。
