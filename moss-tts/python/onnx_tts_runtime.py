...
def ensure_browser_onnx_model_dir(model_dir: str | Path | None = None) -> Path:
    ...
    raise FileNotFoundError(
        "browser_onnx model assets were not prepared by the host. tried: " + ", ".join(tried_paths)
    )
...
class OnnxTtsRuntime(OrtCpuRuntime):
    ...
    def _load_reference_audio(self, reference_audio_path: str | Path) -> np.ndarray:
        resolved_path = Path(reference_audio_path).expanduser().resolve()
        with wave.open(str(resolved_path), "rb") as wav_file:
            ...
            if compression_type != "NONE":
                raise ValueError(f"Reference audio must be uncompressed PCM WAV; got {compression_type}.")
            if sample_width != 2:
                raise ValueError(f"Reference audio must be 16-bit PCM WAV; got {sample_width * 8}-bit audio.")
            raw_frames = wav_file.readframes(frame_count)
        expected_bytes = frame_count * source_channels * sample_width
        if len(raw_frames) != expected_bytes:
            raise ValueError(
                f"Reference audio ended early: expected {expected_bytes} bytes, got {len(raw_frames)} bytes."
            )
        waveform = np.frombuffer(raw_frames, dtype="<i2").reshape(frame_count, source_channels)
        ...
        if sample_rate != target_sample_rate:
            raise ValueError(
                f"Reference audio must use {target_sample_rate} Hz for this minimal runtime; got {sample_rate} Hz."
            )
        ...
        *,
        ...
        def notify_frame(
            ...
            if on_frame is not None:
                on_frame(step_index, len(generated_frames))

        if not streaming:
            generated_frames = self.generate_audio_frames(
                request_rows,
                on_frame=notify_frame,
        ...
        def on_stream_frame(
            ...
            notify_frame(generated_frames, step_index, frame)
    ...
    def synthesize(
        ...
        *,
        ...
        enable_normalize_tts_text: bool = True,
        seed: int | None = None,
        on_frame: Callable[[int, int, int, int], None] | None = None,
        cancel_check: Callable[[], None] | None = None,
    ) -> dict[str, Any]:
        if cancel_check is not None:
            cancel_check()
        ...
        for chunk_index, chunk_text in enumerate(text_chunks):
            ...
            chunk_result = self.synthesize_single_chunk(
                ...
                on_frame=(
                    (lambda step_index, generated_frame_count, chunk_index=chunk_index: on_frame(
                        chunk_index,
                        len(text_chunks),
                        step_index,
