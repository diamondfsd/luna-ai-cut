...
test('真实 NAS 可连接、选择目录并同步本地文件', async ({ lunaApp }) => {
  ...
  await form.getByLabel('服务器地址').fill(process.env.LUNA_NAS_HOST ?? '192.168.31.31')
  await form.getByLabel('端口').fill(process.env.LUNA_NAS_PORT ?? '445')
  await form.getByLabel('用户名').fill(process.env.LUNA_NAS_USERNAME ?? '')
  await form.getByLabel('密码').fill(process.env.LUNA_NAS_PASSWORD ?? '')
  await page.getByRole('button', { name: '连接并选择目录', exact: true }).click()

  const dialog = page.getByRole('dialog', { name: '选择同步目录' })
  await expect(dialog).toBeVisible({ timeout: 30_000 })
  const shareSelect = dialog.getByRole('combobox', { name: '请选择共享目录' })
  ...
  const directorySelect = dialog.getByRole('combobox', { name: '请选择同步目录' })
  ...
  const selectedRemotePath = requestedDirectory === '/' || selectedDirectory === '共享根目录'
    ? '/'
    : selectedDirectory
  ...
  await expect.poll(async () => (await page.evaluate(() => window.luna.getSettings())).nasSync?.remotePath)
    .toBe(selectedRemotePath)

  await page.getByRole('switch', { name: '启用 NAS 同步' }).click()
  await expect.poll(async () => (await page.evaluate(() => window.luna.getSettings())).nasSync)
